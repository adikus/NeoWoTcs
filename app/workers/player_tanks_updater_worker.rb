class PlayerTanksUpdaterWorker
include Sidekiq::Worker
	...
	def perform(region, ids)
		return if region == 4
		#Rails.logger.info "Starting player tanks updater worker with ids #{ids}"

		response = WargamingApi.new(region).tank_stats(ids)
		...
		if response.parsed_response['data'].nil?
			...
			ids.each do |id|
				...
				player.update_tanks(response.parsed_response['data'][id.to_s])
